...
- Fixed `dlba_frechet` and `plba_frechet` returning 0 for `A = 0` (no start-point variability). They now use the closed form for a fixed start point, as the other distributions do (#4).
