...
- Added the shifted Wald distribution in the evidence accumulation parameterization, see `?ShiftedWald`. It is the single Wald accumulator without start point variability and is provided by `dwald`, `pwald`, `qwald`, and `rwald` (which default to `A = 0`), including variability of non-decision time (`st0`) as well as `log`, `lower.tail`, and `log.p` arguments. Suggested by Gidon Frischkorn (#20).
- Added the censored shifted Wald model of Miller, Scherbaum, Heck, Goschke, and Enge (2018), see `?CensoredShiftedWald`: a competing-risks race of two shifted Wald accumulators in which errors censor the correct response process. It is provided by `dcswald`, `pcswald`, `qcswald`, and `rcswald`, which accept either a single response bound `b` (as in the RDM and LBA) or a boundary separation `a` (as in the Wiener diffusion model), plus a relative start point `w`. Suggested by Gidon Frischkorn (#20).
