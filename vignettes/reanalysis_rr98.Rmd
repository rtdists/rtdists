...
The code heavily uses [`dplyr`](https://cran.r-project.org/package=dplyr) ([vignette](https://cran.r-project.org/web/packages/dplyr/vignettes/introduction.html)),  [`tidyr`](https://cran.r-project.org/package=tidyr) ([vignette](https://cran.r-project.org/web/packages/tidyr/vignettes/tidy-data.html)), and [`purrr`](https://cran.r-project.org/package=purrr) for data handling and `lattice` (see `?Lattice`) and `latticeExtra` (specifically `as.layer`) for plotting. A throrough introduction to the former three packages is provided in [R for Data Science](http://r4ds.had.co.nz) by Wickham and Gorlemund, see especially Chapter 25.
