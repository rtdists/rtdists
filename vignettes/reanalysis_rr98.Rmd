...
The code heavily uses [`dplyr`](https://cran.r-project.org/package=dplyr) ([vignette](https://cran.r-project.org/web/packages/dplyr/vignettes/introduction.html)) and [`tidyr`](https://cran.r-project.org/package=tidyr) ([vignette](https://cran.r-project.org/web/packages/tidyr/vignettes/tidy-data.html)) for data handling and `lattice` (see `?Lattice`) and `latticeExtra` (specifically `as.layer`) for plotting. 
...
With these functions in place, we now simply need to loop over participants and items to obtain the fit. We do this (as above) using convenient `dplyr` syntax and `dplyr::do`. To ensure we obtain the maximum likelihood estimate, we run five rounds of model fits (i.e., `n_fits=5`) and use the one with the maximum likelihood value. If we would not do so (as was done in previous versions of this document), we would obtain results that do not correspond to the maximum likelihood estimates but to so-called *local minima*.
...
The following table gives the parameters and the negative summed log-likelihoods (i.e., where smaller is better) obtained from this fit:
