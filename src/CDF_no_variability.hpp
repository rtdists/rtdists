...
// Allocate a new 'struct F_calculator' (without variabilities).
{
    ...
    fc = xnew (struct F_calculator, 1);         // NOTE: MEMORY ALLOCATION: struct F_calculator
    fc->N = N;
    fc->plus = -1;

    data = xnew (struct F_plain_data, 1);       // NOTE: MEMORY ALLOCATION: struct F_plain_data
    data->a  = params->a;
    data->v  = params->v;
    data->t0 = params->t0;
    data->d  = params->d;
    data->dz = params->a/N;
    data->F  = xnew (double, N+1);              // NOTE: MEMORY ALLOCATION: (N+1) doubles
    fc->data = data;
...
//
{
    ...
    if (n-1 > tmp_len)
    {
        ...
        tmp = xrenew(double, tmp, n-1);  // NOTE: MEMORY ALLOCATION
        tmp_len = n-1;
    }
...
// The time step size is 'dt', the space grid size is 'dz'.  */
{
    ...
    tmp_vector = xnew (double, N+1);  // NOTE: MEMORY ALLOCATION
