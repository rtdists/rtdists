...
//
{
    ...
    fc = xnew (F_calculator, 1);     // MEMORY ALLOCATION: struct F_calculator
    fc->N = N;
    fc->plus = -1;
    data = xnew (F_st0_data, 1);     // MEMORY ALLOCATION: struct F_st0_data
    data->st0 = st0;
    data->base_fc = base_fc;
    data->M = M;
    /* data->start is set in F_st0_start */
    data->dt = dt;
    data->values = xnew (double, M*(N+1));  // MEMORY ALLOCATION: M*(N+1) doubles
    data->valid = xnew (char, M);           // MEMORY ALLOCATION: M chars
    data->base = 0;
    data->avg = xnew (double, N+1);         // MEMORY ALLOCATION: N+1 doubles
    fc->data = data;
