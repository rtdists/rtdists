...
F_plain_start (struct F_calculator *fc, enum boundary b)
{
	...
	fc->plus = b;
	data->t_offset = data->t0 - data->d * (b == b_upper ? 0.5 : -0.5);
	data->t = 0;
 ...
 * trapezoidal rule.  Since computing the CDF is slow and since we can
