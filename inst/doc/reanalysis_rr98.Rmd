...
We will follow Ratcliff and Rouder (1998) and analyze the data with the diffusion model. For this, we will fit a separate model to each participant and instruction condition. Like Ratcliff and Rouder we will fit the data to the strength bins instead of the full strength manipulation. We fit basically the full diffusion model (with the excpetion of $s_{t0}$) to each instruction condition which results in 10 parameters per participant and instruction condition:
...
Like in Ratcliff and Rouder (1998), the two boundaries are the two response options "dark" and "light". To estimate the model we diverge from Ratcliff and Rouder and employ trial wise maximum likelihood estimation (i.e., no binning of responses). 

To do so, we simply need to have a wrapper function which returns us the summed log-likelihood of the data (i.e., RTs and corresponding responses) given a set of parameters. The following function for which we simply loop across drift rates will do so:
...
- 5 drift rates $v$ (i.e., one per strength bin)
- 2 boundary separations $A$ (i.e., one for each accumulator)
...
With this, we simply need to loop across participants and instructions to estimate the LBA. Interestingly, we need to run several fitting runs to reach the maximum likelihood estimate (i.e., the global optimum).
...
The following table gives the parameters and the negative summed log-likelihoods obtained from the LBA fit (with $b$ already correctly transformed by the maximum $A$):
...
The log-likelihood shows that the LBA provides a better account for four of the six data sets. The diffusion model only provides a better account for the `kr` and `nh` accuracy conditions. In terms of the parameter estimates we see a pattern similar as the one for the diffusion model:
...
The fact that the LBA provides a better fit is also visible in the graphical fit assessment.

### Predicted Response Rates

We again first consider the predicted response rates (in black) and they are highly accurate.
...
Finally, we graphically compare the fit of the two models.
...
We can see that the LBA is considerbaly more accurate in describing the accuracy condition while the diffusion model seems to be somewhat more accurate for the speed condition, especially for the extreme bins.

## Predicted Median RTs

Next, we also compare the two accounts for the median. As before, data is shown in grey and we begin with a plot for the speed condition, spearated by response.
...
These two plots reval an interesting difference between the models. While the LBA seems to be better able to exactly describe the mejority of the data points it had problems describing the one or two strength bins in which there was a marked difference in response times. In contrast, the diffusion model seemed to be good in adequately describing the pattern of the RTs across strength bin. Unfortunately, the predictions of the diffusion model were shifted by quite a considerably account (> 50 ms) for most participants in line with the log-likelihoods suggesting that overall its account is worse.
