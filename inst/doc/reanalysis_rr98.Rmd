...
Next, we fit the LBA model to the data. For this, we use an LBA model with the same number of parameters. To make the model identifiable, we fix the sum of the drift rates to 1. Specifically, the model has the following 20 parameters per participant:

- 2 $\times$ 5 drift rates (i.e., one per strength bin) = 10 drift rates $v$
- 4 boundary separations $A$ (i.e., one for each accumulator per instruction condition)
- 2 non-decision times $t_0$ (i.e., one per participant)
- 2 drift rate variabilities $s_v$ 
- 2 starting points $b$ (they are parameterized as an increment to the max value of the two $A$ per instruction condition)
...
The following table gives the parameters and the negative summed log-likelihoods obtained from the LBA fit:
...
The log-likelihood shows that the LBA provides a better account for all data sets.
...
Overall the LBA is able to describe the central tendency relatively well. Only at the extreme bins with few responses is some misfit noticeable.
